+++
title = "Gifts Giving"
description = "Living independent?"
date = 2026-01-04
draft = false

[taxonomies]
tags = ["internship"]

[extra]
lang = "en"
toc = true
comment = false
copy = true
outdate_alert = false
outdate_alert_days = 120
math = false
mermaid = false
featured = false
reaction = false
+++

Long time no see!

Tons of apologies for not being regular, but things have become somewhat *chaotic* and *uncertain* in my life. Well, I have landed myself some "*industry exposure*", if you will...

Joined the office cult? Not really. It’s pretty chill around here. But there’s a constant hustle of flying between Delhi, Bengaluru, and home. It does get tiring.

It's still an internship. Rat race? Not really. Hopefully, more good things await. This is my [current feeling](https://www.reddit.com/media?url=https%3A%2F%2Fpreview.redd.it%2Fhaving-a-job-vs-not-having-a-job-v0-ttay3zv675gf1.png%3Fwidth%3D1080%26format%3Dpng%26auto%3Dwebp%26s%3D8b342d1270dd4b4a7f7aa80d4e8c321a6157895a).

{{ figure(src="/img/gifts-giving/lego.avif", alt="something non tech for once", caption="git merge legos") }}

{{ figure(src="/img/gifts-giving/office-lunch.avif", alt="real happy meal", caption="Trying to fix life's biggest bug (a bad diet)") }}

Love doing what I do (though very little). Ah, getting to learn, yes...

I do love the feeling of hearing something technical, maybe a new project, it sounds so cool. Being the smallest name on that contributor's list, it still amazes me. 

Anyway, I have stopped bulking, and I am officially FAT now. I guess it’s difficult to manage everything at once.

{{ figure(src="/img/gifts-giving/b360.avif", alt="the 360 machine", caption="Vibe coding ROCKS (some days)") }}

Is it all about just making money? Never.

Does seeing your first paycheck (your own money) make you happy & less guilty about all your life's choices? **** yeah! It's still just the beginning, and a long way to go. More new places to explore.

Let's see what comes next.

See you soon.
